  ...
  return (
    <section className={`py-20 ${themeClasses}`}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div
          className={`max-w-4xl mx-auto rounded-xl shadow-2xl overflow-hidden ${themeClassesv2}`}
        >
          {/* Tab Header */}
                ...
                <Button
                  ...
                  className={`font-bold ${
                    activeTab === tab.id
                      ...
                      : "text-black hover:text-blue-600 hover:bg-transparent"
                  }`}
                >
                  {tab.label}
                </Button>
            ...
            <div className="grid lg:grid-cols-2 gap-8">
              ...
              <div className="space-y-6">
                {mockCases.map((case_) => (
                  <div
                    key={case_.id}
                    className={`p-4 rounded-lg border ${caseClasses}`}
                  >
                    <div className="flex items-center justify-between mb-2">
                      <span className={isDarkMode ? 'font-bold text-white' : 'font-bold text-black'} >{case_.title}</span>
                      <span
                        className={`font-bold px-2 py-1 rounded text-sm ${
                          isDarkMode ? case_.statusColorDark : case_.statusColor
                        }`}
                      >
                        {case_.status}
                      </span>
                ...
                <div className="space-y-2">
                  {quickActions.map((action, index) => (
                    <Button
                      key={index}
                      variant="ghost"
                      className={`w-full p-3 rounded-lg transition-colors font-bold ${
                        isDarkMode
                          ? "hover:bg-gray-600 border border-gray-600 text-white hover:dark:text-white"
                          : "hover:bg-gray-100 border border-gray-200 text-black hover:dark:text-black"
                      }`}
                    >
